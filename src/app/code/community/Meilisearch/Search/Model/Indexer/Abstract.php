...
abstract class Meilisearch_Search_Model_Indexer_Abstract extends Mage_Index_Model_Indexer_Abstract
{
    /** @var Meilisearch_Search_Model_Resource_Engine */
    protected $engine;

    protected $enableQueueMsg = 'Please enable the queueing system to do it asynchronously (CRON) if you have a lot of products in System > Configuration > Meilisearch Search > Queue configuration';

    public function __construct()
    {
        parent::__construct();

        $this->engine = Mage::getResourceModel('meilisearch_search/engine');
    }

    /**
     * This function will update all the requested categories and their child
     * categories in Meilisearch. You can provide either a single category ID or an
     ...
     * products in Meilisearch. You can provide either a single product ID or an
    ...
    public function reindexSpecificProducts($updateProductIds)
    {
        ...
        if (!empty($productIds)) {
            // Check if queue is active before rebuilding
            $config = Mage::helper('meilisearch_search/config');
            if ($config->isQueueActive()) {
                // Use queue system - this will add to meilisearch_queue
                $this->engine->rebuildProductIndex(null, $productIds);
            } else {
                // Direct indexing without queue
                $helper = Mage::helper('meilisearch_search');
                foreach (Meilisearch_Search_Helper_Entity_Helper::getStores(null) as $storeId) {
                    if ($config->isEnabledBackend($storeId)) {
                        $helper->rebuildStoreProductIndex($storeId, $productIds);
                    }
                }
            }
        }
